...
pub trait ExportTerm<A: ExportAst<C, O, T, S, Y>, C, O, T, Y, S>:
    SaneTerminator<O, T, Y, S>
{
    fn go(
        &self,
        ctx: &mut C,
...
pub fn render_relooped_func<
    C,
    O,
    T: ExportTerm<A, C, O, T, Y, S>,
    Y,
    S,
    A: ReloopAst<C, O, T, S, Y>,
>(
    f: &Func<O, T, Y, S>,
    a: &mut A,
    ctx: &mut C,
...
pub fn render_relooped<C, O, T: ExportTerm<A, C, O, T, Y, S>, Y, S, A: ReloopAst<C, O, T, S, Y>>(
    ...
    match x {
        relooper::ShapedBlock::Simple(s) => {
            ...
            render_bb(ctx, f, s.label, a, |ctx, k| {
                match s.branches.get(&k).unwrap() {
...
pub fn vars<'a, C, O, T: ExportTerm<A, C, O, T, Y, S>, Y, S, A: ExportAst<C, O, T, S, Y>>(
    f: &'a Func<O, T, Y, S>,
) -> impl Iterator<Item = (A::Var, &'a Y, Option<usize>)> + 'a {
    let e = f.entry;
    return f
        .opts
        .iter()
