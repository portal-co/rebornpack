...
use id_arena::Id;
use rat_ir::{
    no_push, Block, BlockTarget, Bound, BoundOp, BoundSelect, BoundTerm, BoundType, Func, Use, Value
};
use swc_core::{
    atoms::Atom,
...
no_push!(type BasicTinyOp<O,T,Y,S>;);
no_push!(type BasicTinyTerm<O,T,Y,S>;);
                        ...
                        match f {
                            Mapper::Func(f, _, decl) => {
                                ...
                                match O::call(*f) {
                                    ...
                                    None => {
                                        let r = n.funcs[a].blocks.alloc(Default::default());
                                        let nb = n.funcs[a].blocks.alloc(Default::default());
                                        for r in args.iter(){
                                            n.funcs[a].add_blockparam(nb, Y::default());
                                        }
                                        func(decl,a,m,n,Arc::new(move|x|T::just(BlockTarget { block: r, args: vec![x], prepend: vec![] })),nb)?;
                                        let v = n.funcs[a].add_blockparam(r, Default::default());
                                        return Ok((v,r));
                                    }
                                }
                            }
