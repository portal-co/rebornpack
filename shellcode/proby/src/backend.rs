...
impl Ast{
    pub fn fold<E>(&self, go: &mut impl FnMut(Ast) -> Result<Ast,E>) -> Result<Ast, E>{
        let r = match self{
            Ast::Call(a, b) => {
                Ast::Call(Box::new(a.fold(go)?), b.iter().map(|c|c.fold(go)).collect::<Result<Vec<_>,_>>()?)
            },
            Ast::Jmp(_, _) => todo!(),
            Ast::Sym(_) => todo!(),
            Ast::Var(_) => todo!(),
            ...
            Ast::Const(_) => todo!(),
            Ast::Bin(_, _, _) => todo!(),
            Ast::If { cond, cty, if_true, if_false } => todo!(),
            Ast::Param(_) => todo!(),
            Ast::Plat(_, _) => todo!(),
            Ast::OS => todo!(),
            Ast::Ret(_) => todo!(),
    ...
    pub fn size(&self, go: &mut usize){
        match self{
            ...
            Ast::If { cond, cty, if_true, if_false } => {
                cond.size(go);
                if_true.size(go);
                if_false.size(go);
            },
    ...
    pub fn vars(&self, go: &mut BTreeSet<String>){
        match self{
            ...
            Ast::If { cond, if_true, if_false, cty } => {
                cond.vars(go);
                if_true.vars(go);
                if_false.vars(go);
            },
